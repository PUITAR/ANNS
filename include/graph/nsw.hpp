  ...
  namespace graph
  {
    ...
    public: // variables
      ...
      NSW(
          ...
          size_t random_seed = 123) : D_(D), max_elements_(max_elements), M_(M), Mmax_(2 * M),
      ...
      SearchBaseLayer(id_t ep_id, const vdim_t *data_point, size_t ef)
      {
      ...
      void
      GetNeighborsByHeuristic(std::priority_queue<std::pair<float, id_t>> &top_candidates, size_t NN)
      {
      ...
      std::priority_queue<std::pair<float, id_t>>
      Search(const vdim_t *query_data, size_t k, size_t ef)
      {
      ...
      std::priority_queue<std::pair<float, id_t>>
      Search(const vdim_t *query_data, size_t k, size_t ef, id_t ep)
      {
        assert(ready_ && "Index uninitialized!");
...
#pragma omp parallel for schedule(dynamic, 64) num_threads(num_threads_)
      ...
      void
      Populate(const std::vector<vdim_t> &raw_data)
      {
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(num_threads_)
      ...
      void
      SetNumThreads(size_t num_threads) { num_threads_ = num_threads; }
